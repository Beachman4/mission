...
switch (_code) do
{
    ...
    case 36:
    {
        ...
        if (!_shift && _ctrlKey) then {
            if (!isNull(findDisplay(2520))) then { closeDialog 0; };
	...
	case 20:
	{
        ...
        if (_shift) then {
            if (playerSide == west) then {
                [cursorTarget] call ffg_fnc_removeLicenseMenu;
                _handled = true;
            };
        };
	};
	...
	case 22:
	{
		if(!_alt && !_ctrlKey) then {
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				if ((typeOf _veh) in ["Land_HouseDoubleAL", "Land_HouseDoubleAL2"]) then {
					if (_veh in life_vehicles && player distance _veh < 18) then {
						...
						if(_locked == 0) then {
							...
							_veh animate [format["Door_%1",_door],0];
							systemChat localize "STR_House_Door_Lock";
						} else {
							_veh setVariable[format["bis_disabled_Door_%1",_door],0,true];
							_veh animate [format["Door_%1",_door],1];
							systemChat localize "STR_House_Door_Unlock";
						};
					};
				} else {
					if(_veh in life_vehicles && player distance _veh < 8) then {
						if ((typeOf _veh) in ["Land_HouseA", "Land_HouseA1", "Land_HouseA1_L", "Land_HouseB", "Land_HouseB1", "Land_HouseB1_L", "Land_HouseC_R", "Land_HouseC1_L"]) then {
							_door = [_veh] call life_fnc_nearestDoor;
							if(_door == 0) exitWith {hint localize "STR_House_Door_NotNear"};
							_locked = _veh getVariable [format["bis_disabled_Door_%1",_door],0];
							if(_locked == 0) then {
							...
							_door = [_veh] call life_fnc_nearestDoor;
							if(_door == 0) exitWith {[localize "STR_House_Door_NotNear",3,"blue"] call A3L_Fnc_Msg;};
							_locked = _veh getVariable [format["bis_disabled_Door_%1",_door],0];
							if(_locked == 0) then {
								_veh setVariable[format["bis_disabled_Door_%1",_door],1,true];
								_veh animate [format["door_%1_rot",_door],0];
                                [localize "STR_House_Door_Lock",3,"blue"] call A3L_Fnc_Msg;
								systemChat localize "STR_House_Door_Lock";
							} else {
								_veh setVariable[format["bis_disabled_Door_%1",_door],0,true];
								_veh animate [format["door_%1_rot",_door],1];
                                [localize "STR_House_Door_Unlock",3,"blue"] call A3L_Fnc_Msg;
								systemChat localize "STR_House_Door_Unlock";
							};
						};
					};
